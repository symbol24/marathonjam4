...
func toggle_pause_menu(display:bool = false) -> void:
	if display:
		...
		if not settings.pending_changes:
			...
			Signals.DisplayPopup.emit(RidPopupManager.Type.LARGE, "pause_settings_pending_changes", RidPopupManager.Severity.WARNING, tr("pause_settings_pending_changes_title"), tr("pause_settings_pending_changes_text"), 0)
